...
const Header = React.forwardRef<HTMLDivElement, HeaderProps>(
  ({ className, sticky, ...props }, ref) => {
    ...
    React.useEffect(() => {
      const mediaQuery = window.matchMedia('(min-width: 1024px)') // lg breakpoint (1024px)
      
      // Define sticky based on media query
      const handleMediaChange = () => setIsLargeScreen(mediaQuery.matches)
      
      // Run on mount
      handleMediaChange()
      
      // Add listener for future screen resizes
    ...
    return (
      <div
        ...
        data-layout="header"
        className={cn(
          `z-10 flex h-[var(--header-height)] items-center gap-4 bg-background/95 shadow backdrop-blur supports-[backdrop-filter]:bg-background/60 dark:shadow-secondary p-4 md:px-8`,
          contextVal.offset > 10 && sticky ? 'shadow' : 'shadow',
          contextVal.fixed && 'flex-none',
          sticky && isLargeScreen ? 'sticky top-0' : '', // Only apply sticky if it's a large screen
          className
        )}
        {...props}
      />
